...
При наступлении некоторых событий (превышение группового лимита, не выполнение оплаты и пр.) 
серверное ПО может сгенерировать запрет на выдачу топлива группам пользователей (организациям).*/
# Пример запроса: {
...
# Пример запроса: {
    ...
    "Begin": 8, // структура, соответствующая конкеретной ТРК
    "End": 8
}
...
/*------ Разделы 6, 7 - новые, на данный момент не реализованы ни в каком виде ------*/

/* 6.ЗАПУСК НАЛИВА:
ПЛК с заданной периодичностью опрашивает сервер на предмет необходимости запуска налива.
При подтверждении команды на запуск, ТРК переходит из состояния "В ожидании" в "Налив".
...
- Налив;
- Завершение налива (ТРК выводит сообщение о количестве выданного нефтепродукта, выдерживает паузу/ждёт нажатия кнопки или сброса с ПО верхнего уровня);
- Ошибка;*/


/*------ Разделы ниже не актуальны для пилотного проекта - "мМодернизация нефтебазы СтрежТрансСервис" ------*/
...
/* ТРАНЗАКЦИИ ПРИХОДА НЕФТЕПРОДУКТА
Каждый из множества ПЛК отправляет информацию о выполненной транзакции сразу после её завершения. 
